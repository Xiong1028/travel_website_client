...
import { ListInfo, ListNote } from "./style";
import "./search.css";
  ...
  render() {
    ...
    if (searchVal) {
      searchList.filter((value, index) => {
        ...
        if (
          re.test(value.post_title) ||
          re.test(value.username) ||
          re.test(value.post_content)
        ) {
          value.post_title = value.post_title.replace(
            re,
          ...
          value.post_content = value.post_content.replace(
            re,
            `<span class="keyword">${searchVal}</span>`
          );
    ...
    return (
      <Container className="searchInfo">
        <span className="feedbackMsg">
          About {searchResultList.length} results
        </span>
